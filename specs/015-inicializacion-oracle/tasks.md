...
- [X] T022 [FR-002] Regenerar mediante Hibernate y revisar `database/generated/piip-oracle.sql` únicamente con autorización explícita, comprobando las 19 tablas, índices, FKs, constraints e identidades sin editar el DDL a mano. Evidencia: el script generado coincide exactamente con el artefacto versionado, incluye las 26 FK con nombres descriptivos (`FK_AUDACCESO_USUARIO`, `FK_CATITEM_CATALOGO`, etc.) derivados de las anotaciones `@ForeignKey` en las entidades JPA, y supera la comparación byte a byte de CI (`cmp`).
...
- [X] T025 [FR-015] Ejecutar la suite backend no integrativa desde `apps/backend` con `gradlew.bat test` para validar perfiles ordinarios y regresiones. Evidencia: la suite completa terminó con 188 tests (187 originales + 1 nuevo `emitsDescriptiveForeignKeyNames` que valida los 26 nombres FK descriptivos), 0 fallos y 0 errores.
- [X] T026 [FR-001] Ejecutar la integración Oracle desde `apps/backend` con `gradlew.bat integrationTest` contra el esquema descartable autorizado y registrar conteos, reejecución, guardias y datos de US3. Evidencia: ejecución completada con `BUILD SUCCESSFUL` en 21 s; 2 tests pasaron (preservación de identidad + reejecución idempotente); Hibernate recreó las 19 tablas y cargó el seed completo.
- [X] T027 [FR-015] Arrancar posteriormente la aplicación con `dev` y con `prod` activado externamente, verificando `ddl-auto=validate`, ausencia de seed, ausencia de creación implícita y validación de `ProductionAdminGuard`. Evidencia: dev arrancado en puerto 4401 con health UP y contexto `/api/v1`; prod arrancado en puerto 4402 con health UP; ambos procesos finalizados limpiamente sin residuos.
