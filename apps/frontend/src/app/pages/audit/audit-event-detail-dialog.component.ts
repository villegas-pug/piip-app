  ...
  template: `
    <div class="audit-detail-dialog">
      ...
      <mat-dialog-content>
        ...
        @if (event.status || event.previousStatus || event.newStatus) {<section class="event-fields"><h3>Estados</h3><dl>@if (event.status) {<div><dt>Estado</dt><dd>{{ event.status.name || 'Estado no disponible' }}</dd></div>} @if (event.previousStatus) {<div><dt>Estado anterior</dt><dd>{{ event.previousStatus.name || 'Estado no disponible' }}</dd></div>} @if (event.newStatus) {<div><dt>Estado nuevo</dt><dd>{{ event.newStatus.name || 'Estado no disponible' }}</dd></div>}</dl></section>}
        @if (event.detailFields.length) {<section class="event-fields"><h3>Datos del evento</h3><dl>@for (field of event.detailFields; track field.label + field.value + $index) {<div><dt>{{ field.label }}</dt><dd>{{ field.value }}</dd></div>}</dl></section>}
        <details><summary>Datos técnicos</summary><p>Sujeto técnico: <code>{{ event.source.actorSubject ?? 'No registrado' }}</code></p><pre>{{ event.technicalDetail }}</pre></details>
      </mat-dialog-content>
