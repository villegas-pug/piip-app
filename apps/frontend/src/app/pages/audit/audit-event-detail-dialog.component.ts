  ...
  template: `
    <div class="audit-detail-dialog">
      ...
      <mat-dialog-content>
        ...
        @if (event.status || event.previousStatus || event.newStatus) {<section class="event-fields"><h3>Estados estructurados</h3><dl>@if (event.status) {<div><dt>Estado</dt><dd><code>{{ event.status.code }}</code> · {{ event.status.name }}</dd></div>} @if (event.previousStatus) {<div><dt>Estado anterior</dt><dd><code>{{ event.previousStatus.code }}</code> · {{ event.previousStatus.name }}</dd></div>} @if (event.newStatus) {<div><dt>Estado nuevo</dt><dd><code>{{ event.newStatus.code }}</code> · {{ event.newStatus.name }}</dd></div>}</dl></section>}
