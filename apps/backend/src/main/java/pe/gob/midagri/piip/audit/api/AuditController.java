...
public class AuditController {
    ...
    @GetMapping("/events") @Transactional(readOnly = true) public List<EventResponse> events() { authorization.require(RoleCode.ADMINISTRADOR_PIIP); return events.findTop100ByOrderByOccurredAtDesc().stream().map(AuditController::toEventResponse).toList(); }
